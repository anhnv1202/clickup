var OneTrustStub = function(t) {
    ...
    var d = new function() {
        ...
        m = (h.prototype.initConsentSDK = function() {
            ...
            p.stubScriptElement && p.stubScriptElement.hasAttribute(p.DATAFILEATTRIBUTE) ? this.domainId = p.stubScriptElement.getAttribute(p.DATAFILEATTRIBUTE).trim() : t ? this.domainId = p.stubScriptElement.getAttribute("src").split("did=")[1] : p.stubScriptElement || (p.stubScriptElement = document.querySelector("script[src*='" + p.migratedCCTID + "']"), p.stubScriptElement && (p.isMigratedURL = !0, this.domainId = p.migratedDomainId.trim()))
            ...
            "debug" !== this.buildType && "cybuild" !== this.buildType || (t.Version = "202403.1.0")
        }, h);
